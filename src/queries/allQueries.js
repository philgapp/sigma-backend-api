...
module.exports = {
    Query: {
        ...
        getDashboard: async ( root, args, ctx ) => {
            ...
            let finalDashboard
            const bankingArray = ( await ctx.Banking
                ...
                .then( async ( error, result ) => {
                    if ( error ) {
                        ...
                        if ( result ) {
                            ...
                            const openOptions = ( await ctx.Options
                                .find( {
                                    $and: [
                                        { userId: { $eq: userId } },
                                        { endDate: null } ] } )
        ...
        getUnderlying: async ( root, args, ctx ) => {
            ...
            const res = ( await ctx.Underlying
                .find( query )
                .toArray() )
                    .map( prepare )
            return res
